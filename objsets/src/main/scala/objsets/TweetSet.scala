...
object Nil extends TweetList {
  ...
  def isEmpty = true
  def addAsLast(tweet: Tweet): TweetList = new Cons(tweet, Nil)
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => tweetContainsKeyWord(tweet, google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => tweetContainsKeyWord(tweet, apple))
  ...
  private def tweetContainsKeyWord(tweet: Tweet, keyWords: List[String]): Boolean =
    if(keyWords.isEmpty) false
    else tweet.text.contains(keyWords.head) || tweetContainsKeyWord(tweet, keyWords.tail)
